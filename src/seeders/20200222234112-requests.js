module.exports = {
  up: (queryInterface) => queryInterface.bulkInsert(
    'Requests', [
      {
        id: '51e74db7-5510-4f50-9f15-e23710331ld5',
        userId: '79660e6f-4b7d-4g21-81re-74f54e9e1c8a',
        location: 'nairobi',
        destination: 'nairobi',
        reason: 'meeting with partners',
        departureDate: '2020-01-01',
        managerId: '0119b84a-99a4-41c0-8a0e-6e0b6c385165',
        email: 'jeanne@andela.com',
        type: 'two way',
        status: 'open',
        confirm: false,
        returnDate: '2020-02-01',
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        id: 't1e74db7-h610-4f50-9f45-e2371j331ld5',
        userId: '79660e6f-4b7d-4g21-81re-74f54jk91c8a',
        location: 'boston',
        destination: 'kigali',
        reason: 'Flying to Japan for tech ceremony',
        departureDate: '2017-05-01',
        email: 'jdev@andela.com',
        type: 'two way',
        status: 'approved',
        managerId: "0119b84a-99a4-41c0-8a0e-6e0b6c385165",
        confirm: false,
        returnDate: '2017-06-15',
        createdAt:  "2010-02-01T13:46:04.162Z",
        updatedAt: new Date(),
      },
      {
        id: 't1e74db7-h610-4f50-9f45-e2371j331ld4',
        userId: '79660e6f-4b7d-4g21-81re-74f54jk91c8a',
        location: 'boston',
        destination: 'gisenyi',
        reason: 'Having fun with familty at any restaurant',
        departureDate: '2020-07-01',
        email: 'jdev@andela.com',
        type: 'two way',
        status: 'rejected',
        returnDate: '2021-02-15',
        confirm: true,
        managerId: '0119b84a-99a4-41c0-8a0e-6e0b6c385165',
        createdAt:  "2020-03-08T13:46:04.162Z",
        updatedAt: new Date(),
      },
      {
        id: 't1e74db7-h610-4f50-9f45-e2371j331ld9',
        userId: '79660e6f-4b7d-4g21-81re-74f54jk91c8a',
        location: 'boston',
        destination: 'gisenyi',
        reason: 'Investors meeting in USA',
        departureDate: '2020-12-01',
        email: 'jdev@andela.com',
        type: 'one way',
        status: 'rejected',
        confirm: false,
        returnDate: '2021-02-15',
        managerId: '0119b84a-99a4-41c0-8a0e-6e0b6c385165',
        createdAt:  "2018-01-06T13:46:04.162Z",
        updatedAt: new Date(),
      },
      {
        id: '0ad0ef9d-a926-4c5c-86e6-4d1e22e9ab88',
        userId: 'aed6785b-382f-43a7-a0b5-15a78dd02cc7',
        location: 'boston',
        destination: 'bisenyi',
        reason: 'meeting with engineers write',
        departureDate: '2017-04-01',
        email: 'rejectuser@andela.com',
        status: 'open',
        type: 'two way',
        confirm: false,
        returnDate: '2017-06-15',
        managerId: '79660e6f-4b7d-4d21-81ad-74f64e9e1c8a',
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        id: 'f88ed93c-b84d-4159-85e0-221cd7b2f547',
        location: 'boston',
        userId: 'aed6785b-382f-43a7-a0b5-15a78dd02cc7',
        destination: 'bisenyi',
        reason: 'meeting with engineers',
        departureDate: '2020-12-01',
        email: 'rejectuser@andela.com',
        status: 'open',
        type: 'two way',
        confirm: false,
        returnDate: '2021-02-15',
        managerId: '0119b84a-99a4-41c0-8a0e-6e0b6c385165',
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        id: '5cb6bd43-f4ab-4fd9-86c6-37075e4f7957',
        location: 'boston',
        destination: 'Nairobi',
        reason: 'Strategy meeting with Human resources officer',
        departureDate: '2018-10-01',
        email: 'jdev@andela.com',
        userId: '79660e6f-4b7d-4g21-81re-74f54jk91c8a',
        status: 'open',
        type: 'two way',
        confirm: false,
        returnDate: '2018-11-20',
        managerId: '0119b84a-99a4-41c0-8a0e-6e0b6c385165',
        createdAt: "2020-01-10T13:46:04.162Z",
        updatedAt: new Date(),
      },
      {
        id: 'b7600a6c-58f8-4de3-9e7d-aa9d83b8b37b',
        location: 'boston',
        userId: 'a3426db1-1d2f-4c2c-8f25-c99962aff3fe',
        destination: 'bisenyi',
        reason: 'the reason is obvious needless to say it again',
        departureDate: '2020-10-01',
        email: 'jean@okay.com',
        status: 'open',
        type: 'two way',
        confirm: false,
        returnDate: '2021-02-15',
        managerId: '79660e6f-4b7d-4d21-81ad-74f64e9e1c8a',
        createdAt: new Date(),
        updatedAt: new Date(),
      }], 
    {},
  ),

  down: (queryInterface) => queryInterface.bulkDelete('Requests', null, {}),
};
